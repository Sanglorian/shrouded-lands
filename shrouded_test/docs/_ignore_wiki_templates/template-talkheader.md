---
layout: wiki_page
title: Template:Talkheader
pageid: 1909
namespace: 10
original_url: https://shrouded-lands.fandom.com/wiki/Template:Talkheader
categories:
- Category:General wiki templates
media: []
---
{| style="width:80%; margin:0 auto 1em auto; border: 1px solid #aaaaaa; background-color:#f9f9f9; color:#000;" align="center" cellpadding="4"
|-
! colspan="2" style="text-align:center; font-size:120%; background-color:#3366CC; color:#ffffff;" |
{{#ifeq:{{NAMESPACE}}|User talk|
This is {{PAGENAME}}'s talk page, where you can leave messages and comments for {{PAGENAME}}.
|
This is the talk page for discussing {{{1|improvements to the :{{SUBJECTPAGENAME}} article}}}.
}}
|-
|
* '''Please sign and date your posts''' by typing four tildes  (<tt><nowiki>~~~~</nowiki></tt>).
* '''Put new text under old text.''' [{{fullurl:{{TALKPAGENAMEE}}|action=edit&section=new}} Click here to start a new topic].
* If you're new to the wiki, please take a look at the '''talk page help'''.
| class="title" style="text-align:left;" |
* Be polite
* Assume good faith
* Be welcoming
|}<noinclude>
{{documentation}}</noinclude>
